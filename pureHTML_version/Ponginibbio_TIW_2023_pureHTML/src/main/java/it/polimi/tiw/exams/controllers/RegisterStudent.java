...
public class RegisterStudent extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// Once the student is registered is redirected to the LoginPage
	...
	private Integer registerStudent(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (name.length() <= 0 || name.length() > 45) {
			...
			forward(request, response, PathHelper.pathToRegisterStudentPage);
			return null;
		}
		...
		if (degree_course.length() <= 0 || degree_course.length() > 45) {
			request.setAttribute("warning", "Chosen degree course invalid (a valid degree course has more than one character and less than 45)!");
			forward(request, response, PathHelper.pathToRegisterStudentPage);
			return null;
		}
		...
		if (user == null) {
			forwardToErrorPage(request, response, "Error: student not correctly created");
			return null;
		}
		return 0;
	}
