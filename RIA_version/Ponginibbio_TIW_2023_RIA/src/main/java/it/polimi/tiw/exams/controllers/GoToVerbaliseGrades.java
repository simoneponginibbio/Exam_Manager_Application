...
import it.polimi.tiw.exams.packets.PacketReport;
...
public class GoToVerbaliseGrades extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (rep == null) {
			response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);		
			response.getWriter().println("Null report, when accessing report details");
			return;
		}
		...
		timestamp.setTimeZone(TimeZone.getTimeZone("UTC+2"));
		timestamp.format(((StudentReport)report.get(0)[0]).getReport().getTimestamp());
		String json = new Gson().toJson(new PacketReport(courseName, examDate, timestamp, report));
